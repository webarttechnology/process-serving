$(document).ready(function(){//enable or disable inpfields in case info
	...
	//var textbox = document.getElementById("exampleInputEmail1");
	//textbox.addEventListener("change", function() {
        ...
        if(caseno.length>3)
        {
            ...
            $.ajax({
                ...
                type	:"POST",
                url		:'searchcase.php',
                data	:{caseno:caseno,clientid:clientid},		
                success:function(data)
                {

					//console.log(data);
					var caseparse = JSON.parse(data);
					console.log(caseparse);
					//var totlcases= JSON.parse(data);
					//console.log(totlcases);
					...
					var i=0;
					caseres = caseparse.Case;
					//let result = Array.isArray(caseres);
					//console.log(caseres.length);
					if((Array.isArray(caseres)))
					{
						if(caseres.length>0)
						{
							$.each(caseres, function (key, val) {
								//alert(key + val);
								$('#caserestbody').append('<tr><th class="getcase" id="case'+i+'" scope="row">'+val.CourtCaseNo+'</th><td id="plain'+i+'" style="color:black;">'+val.Plaintiff+'</td><td id="def'+i+'" style="color:black;">'+val.Defendant+'<input type="text" id="defrol'+i+'" value="'+val.DefendantRole+'" hidden /></td><td id="court'+i+'" style="color:black;">'+val.Court+'</td></tr>');
								i++;
							});  
						}
						else
					...
					{
						if(!$.isEmptyObject(caseres))
						{
							console.log(caseres);
							console.log(caseres.CaseID);
							if((caseres.CaseID!="") && (caseres.CaseID!="undefined"))
							{

								$('#caserestbody').append('<tr><th class="getcase" id="case'+i+'" scope="row">'+caseres.CourtCaseNo+'</th><td id="plain'+i+'" style="color:black;">'+caseres.Plaintiff+'</td><td id="def'+i+'" style="color:black;">'+caseres.Defendant+'<input type="text" id="defrol'+i+'" value="'+caseres.DefendantRole+'" hidden /></td><td id="court'+i+'" style="color:black;">'+caseres.Court+'</td></tr>');
								i++;
                    ...
                    {
						$('#caserestbody').append('<tr><th class="getcase" id="case'+i+'" scope="row">'+caseres[i]["CourtCaseNo"]+'</th><td id="plain'+i+'" style="color:black;">'+Plaintiff+'</td><td id="def'+i+'" style="color:black;">'+caseres[i]["Defendant"]+'<input type="text" id="defrol'+i+'" value="'+DefendantRole+'" hidden /></td><td id="court'+i+'" style="color:black;">'+Court+'</td></tr>');
					}  */  
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                if(serveenum>0)
                {
                    ...
                    if(serveenum==1)
                    {
                        if(xmlDoc.getElementsByTagName("JobNum")[0].childNodes[0])
                        {
        ...
        if(newadr!="" && newcity!="" && newstate!="" && newzip!="")
        {
            ...
            $.ajax({
                ...
                error: function(XMLHttpRequest, textStatus, errorThrown) { 
                    setTimeout(function(){
                        swal("Unexpected error occured. Please try again.");
                        $(".load").hide();
                    }, 10000);
                } 
            });
                                                ...
                                                '<div class="content" style="margin-bottom:10px;">'+
                                                    ...
                                                    '<div class="row mb-3 mt-4">'+
                                                        '<div class="col-md-6">'+
                                                            ...
                                                            '</select>'+
                                                        '</div>'+
                                                        '<div class="col-md-6 mb-3 hrdate" style="padding-top:40px;">'+
